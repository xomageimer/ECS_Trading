...
inline void CheckLimitsSystem(entt::registry& registry)
{
    ...
    // 1) «Если сделка стала RetiredTradeTag», то убавляем лимит на её объём
    {
    ...
    // 2) Считаем объём у активных ордеров
    {
    ...
    // 3) Считаем объём у сделок (TradeTag), которые НЕ имеют RetiredTradeTag
    {
...
inline void MatchingSystem(entt::registry& registry)
{
    ...
    std::vector<entt::entity> matchedBuyOrders;
    std::vector<entt::entity> matchedSellOrders;

    auto view = registry.view<OrderTag, ActiveTag, Side, Price, Quantity>(entt::exclude<FilledTag>);

    for (auto buyEntity : view)
    {
        auto &buySide = view.get<Side>(buyEntity);
        if (buySide.value != SideT::Buy) continue;

        auto &buyPrice = view.get<Price>(buyEntity);
        auto &buyQty   = view.get<Quantity>(buyEntity);

        for (auto sellEntity : view)
        {
            auto &sellSide = view.get<Side>(sellEntity);
            if (sellSide.value != SideT::Sell) continue;

            auto &sellPrice = view.get<Price>(sellEntity);
            auto &sellQty   = view.get<Quantity>(sellEntity);

            if (buyPrice.value >= sellPrice.value)
            {
                int matchQty = std::min(buyQty.value, sellQty.value);

                std::cout << "[MatchingSystem] Match: Buy[" << int(buyEntity)
                          << "] vs Sell[" << int(sellEntity)
                          << "] => Qty=" << matchQty << "\n";

                buyQty.value -= matchQty;
                sellQty.value -= matchQty;

                if (buyQty.value == 0)
                {
                    registry.emplace<FilledTag>(buyEntity);
                    matchedBuyOrders.push_back(buyEntity);
                }
                if (sellQty.value == 0)
                {
                    registry.emplace<FilledTag>(sellEntity);
                    matchedSellOrders.push_back(sellEntity);
                }

                break; // Один sellEntity на один buyEntity
            }
        }
    }

    for (auto e : matchedBuyOrders)
    {
        registry.remove<ActiveTag>(e);
    }
    for (auto e : matchedSellOrders)
    {
        registry.remove<ActiveTag>(e);
    }
}
